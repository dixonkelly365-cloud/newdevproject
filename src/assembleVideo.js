...
async function assemble() {
  ...
  for (const scene of storyboard.scenes) {
    ...
    const clipPath = path.resolve('output/photos', `${scene.id}_a.mp4`);
    const audioPath = path.resolve(normDir, `${scene.id}.mp3`);
    const sceneOut = path.join(scenesDir, `${scene.id}.mp4`);
    const hasAnimatedClip = fs.existsSync(clipPath);
    const hasSplit = fs.existsSync(photoPathB);

    if (!hasAnimatedClip && !fs.existsSync(photoPathA)) {
      throw new Error(`Missing visual for scene "${scene.id}" — run the fetch-visuals step first.`);
    }
    ...
    if (hasAnimatedClip) {
      // Character scene with real bounce/blink animation (from
      // fetchVisuals.js) — loop the short animated clip to fill however
      // long the narration takes, rather than applying Ken Burns (it's
      // already moving).
      console.log(`🎞  Assembling scene "${scene.id}" (animated character loop, ${targetDuration.toFixed(2)}s)`);
      silentVideoPath = path.join(scenesDir, `${scene.id}_silent.mp4`);
      run(
        `ffmpeg -y -stream_loop -1 -i "${clipPath}" ` +
        `-an -c:v libx264 -pix_fmt yuv420p -t ${targetDuration.toFixed(2)} "${silentVideoPath}"`
      );
    } else if (hasSplit) {
      ...
      console.log(`🎞  Assembling scene "${scene.id}" (A/B split, Ken Burns, ${targetDuration.toFixed(2)}s)`);
      ...
      console.log(`🎞  Assembling scene "${scene.id}" (single photo, Ken Burns, ${targetDuration.toFixed(2)}s)`);
      silentVideoPath = path.join(scenesDir, `${scene.id}_silent.mp4`);
